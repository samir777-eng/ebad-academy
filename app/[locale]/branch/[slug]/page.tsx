...
export default async function BranchPage({
  ...
  const filterLevelId = levelParam ? parseInt(levelParam) : null;
  ...
  const branch = await prisma.branch.findUnique({
    ...
    include: {
      lessons: {
        where: filterLevelId ? { levelId: filterLevelId } : undefined,
        include: {
          level: true,
          userProgress: {
  ...
  return (
    <div className="container mx-auto px-6 py-8 max-w-7xl">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-6">
        ...
        {filterLevelId && (
          <span className="ml-2 px-2 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-md text-xs font-semibold">
            Level {filterLevelId}
          </span>
        )}
      </nav>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-600 via-primary-500 to-secondary-600 p-8 shadow-lg mb-8 group">
        ...
        <div className="relative">
          <h1 className="text-4xl font-bold text-white mb-6">{branchName}</h1>
